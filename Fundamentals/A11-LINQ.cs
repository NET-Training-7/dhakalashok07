...
class LINQ
{
    ...
    {
        new Person(){Natioanality="Nepal", Name = "Ashok Dhakal", Dob = new DateTime(2003, 12, 13)},
        new Person(){Natioanality="China", Name = "Ridam", Dob = new DateTime(2020, 9, 13)},
        new Person(){Natioanality="India", Name = "Krishna", Dob = new DateTime(1988, 2, 23)},
        new Person(){Natioanality="USA", Name = "Ridam", Dob = new DateTime(1978, 3, 1)},
        new Person(){Natioanality="India", Name = "Ishan", Dob = new DateTime(1994, 12, 13)},
        new Person(){Natioanality="Nepal", Name = "Sandip", Dob = new DateTime(2008, 10, 29)},
    };
...
new() {Id = 3, Major = "Education", Title = "Med",University = "TU"},
    ...
    public void LearnTOQueryComplexCollections()
    {
       ...
       //            select p.Name;
       var voteList = from p in people
              // where DateTime.Now.Year - p.Dob.Year >= 18
              where ((DateTime.Now -p.Dob).TotalDays/365) >= 18
               select p.Name;


        // Find all nepalese people who born after .NET is released
        var NepaleseBornAfterDotNet = from p in people
                              where p.Natioanality == "Nepal" &&
                                    p.Dob > new DateTime(2002, 1, 1)
                              select p.Name;

                  //. Find all people names and correspoding degree title who has science major
        ...
        //  Find all indians who are doing arts. Print all people and their education dtails. 
